    ...
    public class DataAccess :IDataAccess
    {
        ...
        public Offer GetOffer(int id)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT * FROM Offer WHERE OfferId=" + id + ";";
        ...
        public ProductCategory GetProductCategory(int id)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT * FROM ProductCategory WHERE CategoryId=" + id + ";";
                command.CommandText = query;
                ...
                while (r.Read())
                {
                    productCategory.CategoryId = (int)r["CategoryId"];
        ...
        public List<Product> GetProducts(string category, string subcategory, int count)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT TOP " + count + " * FROM Product WHERE CategoryId=(SELECT CategoryId FROM ProductCategory WHERE Category=@c AND SubCategory=@s) ORDER BY newid();";
                command.CommandText = query;
        ...
        public bool InsertUser(User user)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                query = "INSERT INTO [User] (FirstName, LastName, Address, Mobile, Email, Password, CreatedAt, ModifiedAt) values (@fn, @ln, @add, @mb, @em, @pwd, @cat, @mat);";
        ...
        public void InsertReview(Review review)
        {
            ...
            string query = "INSERT INTO Review (UserId, ProductId, Review, CreatedAt) VALUES (@uid, @pid, @rv, @cat);";
            command.CommandText = query;
            command.Parameters.Add("@uid", System.Data.SqlDbType.Int).Value = review.User.Id;
            command.Parameters.Add("@pid", System.Data.SqlDbType.Int).Value = review.Product.ProductId;
        ...
        public bool InsertCartItem(int userId, int productId)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                connection.Open();
                string query = "SELECT COUNT(*) FROM Cart WHERE UserId=" + userId + " AND Ordered='false';";
                command.CommandText = query;
                int count = (int)command.ExecuteScalar();
                if (count == 0)
                {
                    query = "INSERT INTO Cart (UserId, Ordered, OrderedOn) VALUES (" + userId + ", 'false', '');";
                    command.CommandText = query;
                    command.ExecuteNonQuery();
                }

                query = "SELECT CartId FROM Cart WHERE UserId=" + userId + " AND Ordered='false';";
                command.CommandText = query;
                int cartId = (int)command.ExecuteScalar();


                query = "INSERT INTO CartItem (CartId, ProductId) VALUES (" + cartId + ", " + productId + ");";
                command.CommandText = query;
                command.ExecuteNonQuery();
                return true;
            }
        ...
        public List<Cart> GetAllPreviousCartsOfUser(int userid)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT CartId FROM Cart WHERE UserId=" + userid + " AND Ordered='true';";
                command.CommandText = query;
                connection.Open();
                SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
        ...
        public int InsertPayment(Payment payment)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = @"INSERT INTO Payment (PaymentMethodId, UserId, TotalAmount, ShippingCharges, AmountReduced, AmountPaid, CreatedAt) 
                ...
                if (value > 0)
                {
                    query = "SELECT TOP 1 Id FROM Payment ORDER BY Id DESC;";
                    command.CommandText = query;
        ...
        public int InsertOrder(Order order)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "INSERT INTO [Order] (UserId, CartId, PaymentId, CreatedAt) values (@uid, @cid, @pid, @cat);";
                ...
                if (value > 0)
                {
                    query = "UPDATE Cart SET Ordered='true', OrderedOn='" + DateTime.Now.ToString(dateformat) + "' WHERE CartId=" + order.Cart.Id + ";";
                    command.CommandText = query;
                    command.ExecuteNonQuery();

                    query = "SELECT TOP 1 Id FROM [Order] ORDER BY Id DESC;";
                    command.CommandText = query;
                    value = (int)command.ExecuteScalar();
                }
                else
        ...
        public bool DeleteCartItem(int cartItemId)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                string query = "SELECT COUNT(*) FROM CartItem WHERE CartItemId=@cartItemId;";
                command.CommandText = query;
                ...
                if (count == 0)
                {
                    // Cart item does not exist, return false
                    return false;
                }
        ...
        public bool UpdateUser(User user)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                command.Parameters.Add("@mat", SqlDbType.NVarChar).Value = DateTime.Now.ToString(dateformat);
                command.ExecuteNonQuery();
            }
